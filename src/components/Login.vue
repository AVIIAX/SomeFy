<template>
  ...
  <div class="glitch flex items-center justify-center min-h-screen">
    <div class="w-full max-w-sm rounded-lg shadow-lg p-8">
      ...
      <div class="space-y-4">
        <input type="text" placeholder="Email" v-model="email" @keydown.enter="login"
        ...
        <button @click="login" class="w-full p-3 text-sm font-medium text-white minimal-btn text-center justify-center"
      ...
      <p class="text-center mt-4 text-gray-400 text-sm">
        ...
        <a @click.prevent="$emit('switchToRegister')" href="#" class="text-red-500 hover:underline">
          Register here
        </a>
      </p>
    </div>
...
const signInWithGoogle = async () => {
  ...
  try {
    ...
    // Access user info after successful login
    ...
    // You can also store user data in Firestore or Vue state if needed
  } catch (error) {
